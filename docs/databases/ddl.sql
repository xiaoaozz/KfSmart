...
CREATE TABLE agents (
    ...
    skill_refs      TEXT COMMENT '关联 Skills 技能，逗号分隔 ID',
...
CREATE TABLE skill_definitions (
    ...
    status          VARCHAR(20) NOT NULL DEFAULT '草稿' COMMENT '状态：草稿/已发布/已停用',
    owner_name      VARCHAR(100) COMMENT '负责人',
    description     VARCHAR(500) COMMENT '技能说明',
    tags            TEXT COMMENT '标签，逗号分隔',
    instruction     LONGTEXT COMMENT '技能执行说明',
    system_prompt   LONGTEXT COMMENT '技能级系统提示词',
    input_schema    LONGTEXT COMMENT '输入结构定义',
    output_schema   LONGTEXT COMMENT '输出结构定义',
    runtime_config  LONGTEXT COMMENT '运行参数配置',
    example_input   LONGTEXT COMMENT '示例输入',
    example_output  LONGTEXT COMMENT '示例输出',
    prompt_refs     TEXT COMMENT '关联 Prompt 模板 ID，逗号分隔',
    mcp_tool_refs   TEXT COMMENT '关联 MCP 工具 ID，逗号分隔',
    version         VARCHAR(20) NOT NULL DEFAULT 'v1' COMMENT '版本号',
    call_count      BIGINT NOT NULL DEFAULT 0 COMMENT '调用次数',
    success_count   BIGINT NOT NULL DEFAULT 0 COMMENT '成功次数',
    avg_duration_ms BIGINT NOT NULL DEFAULT 0 COMMENT '平均耗时(ms)',
    published_at    TIMESTAMP NULL COMMENT '发布时间',
    created_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Skills 技能定义表';
...
CREATE TABLE skill_version_histories (
    ...
    name             VARCHAR(100) NOT NULL COMMENT '技能名称',
    category         VARCHAR(50) COMMENT '技能分类',
    version          VARCHAR(20) NOT NULL COMMENT '快照版本号',
    status           VARCHAR(20) COMMENT '快照状态',
    owner_name       VARCHAR(100) COMMENT '负责人',
    description      VARCHAR(500) COMMENT '技能描述',
    tags             TEXT COMMENT '标签',
    instruction      LONGTEXT COMMENT '执行说明',
    system_prompt    LONGTEXT COMMENT 'System Prompt',
    input_schema     LONGTEXT COMMENT '输入结构定义',
    output_schema    LONGTEXT COMMENT '输出结构定义',
    runtime_config   LONGTEXT COMMENT '运行参数配置',
    example_input    LONGTEXT COMMENT '示例输入',
    example_output   LONGTEXT COMMENT '示例输出',
    prompt_refs      TEXT COMMENT 'Prompt 模板引用',
    mcp_tool_refs    TEXT COMMENT 'MCP 工具引用',
    snapshot_by      VARCHAR(100) COMMENT '快照操作人',
    change_description VARCHAR(500) COMMENT '变更说明',
    snapshot_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '快照时间',
    INDEX idx_skill_history_skill_id (skill_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Skills 版本历史表';
...
CREATE TABLE agent_versions (
    ...
    skill_refs      TEXT COMMENT 'Skills 技能快照',
