...
async function savePrompt() {
  ...
  saving.value = true;
  const { data, error } = await fetchSavePromptTemplate(editForm.value);
  saving.value = false;
  if (!error) {
    window.$message?.success(isEdit.value ? '更新成功' : '创建成功');
    // 保存后刷新表单数据为服务端返回值，停留在编辑页面
    if (data) {
      editForm.value = { ...data };
    }
    isEdit.value = true;
...
// 鼠标移开编辑框时，从 System Prompt + User Prompt 内容中提取 {{变量}} 同步到变量字段
          ...
          <div class="w-360px flex-shrink-0 border-l border-gray-200 dark:border-gray-700 bg-white dark:bg-[#18181c] flex flex-col">
            <NScrollbar class="flex-1">
              <div class="p-5 space-y-5">
                ...
                <!-- 版本 -->
                <div>
                  <div class="mb-1.5 text-xs font-medium text-gray-500 dark:text-gray-400">版本</div>
                  <NInput v-model:value="editForm.version" placeholder="例如 v1.0" />
                </div>
