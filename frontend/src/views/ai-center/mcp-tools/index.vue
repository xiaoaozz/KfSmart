...
const tools = ref<Api.AgentCenter.McpTool[]>([]);
...
const columns: DataTableColumns<Api.AgentCenter.McpTool> = [
...
function editTool(row: Api.AgentCenter.McpTool) {
...
async function loadData() {
  loading.value = true;
  try {
    const { error, data } = await fetchMcpTools({ page: 1, size: 100 });
    if (!error && data) tools.value = data.records || data.content || data.data || [];
  } finally {
    loading.value = false;
  }
}
...
<template>
  <div class="h-full overflow-y-auto bg-gray-50 px-8 py-6 dark:bg-gray-900">
    ...
    <div class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
      <NDataTable :columns="columns" :data="tools" :loading="loading" :pagination="{ pageSize: 10 }" size="small" striped />
    </div>
